...
def Block(U, inp):
    ...
    shortcut3x3 = conv2d_bn(inp, int(W * 0.167), 1, 1, activation=None, padding='same')
    conv3x3 = add([conv3x3, shortcut3x3])
    conv3x3 = Activation('relu')(conv3x3)
    conv3x3 = BatchNormalization(axis=3)(conv3x3)

    conv5x5 = conv2d_bn(conv3x3, int(W * 0.333), 3, 3, activation='relu', padding='same')
    shortcut5x5 = conv2d_bn(conv3x3, int(W * 0.333), 1, 1, activation=None, padding='same')
    conv5x5 = add([conv5x5, shortcut5x5])
    conv5x5 = Activation('relu')(conv5x5)
    conv5x5 = BatchNormalization(axis=3)(conv5x5)


    conv7x7 = conv2d_bn(conv5x5, int(W * 0.5), 3, 3, activation='relu', padding='same')
    shortcut7x7 = conv2d_bn(conv5x5, int(W * 0.5), 1, 1, activation=None, padding='same')
    conv7x7 = add([conv7x7, shortcut7x7])
    conv7x7 = Activation('relu')(conv7x7)
    conv7x7 = BatchNormalization(axis=3)(conv7x7)
